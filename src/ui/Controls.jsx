...
export const Btn = ({ children, onClick, variant = "ghost", disabled, glow, style, title }) => (
  <button
    type="button"
    title={title}
    disabled={!!disabled}
    ...
    style={{
      padding: "9px 12px",
      borderRadius: 12,
      border: variant === "primary" ? "1px solid rgba(255,255,255,0.2)" : "1px solid rgba(255,255,255,0.12)",
      background: disabled
        ? "#2a2f3a"
        : variant === "primary"
      ...
      boxShadow:
        variant === "primary"
          ? `0 10px 24px rgba(58,104,255,0.35)${glow ? ", 0 0 18px rgba(58,104,255,0.5)" : ""}`
...
export const IconBtn = ({ label, onClick, title }) => (
  <button
    type="button"
    title={title || label}
    onMouseDown={stop}
